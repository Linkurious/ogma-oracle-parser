...
*/
 ...
 -- prepare string inside LATERAL()
      ...
      SUB_QUERY_STRING := 'select json_object (''id'' value json_value(VT.COLUMN_VALUE, ''$.ELEM_TABLE'') || json_query(VT.COLUMN_VALUE, ''$.KEY_VALUE''), ''properties'' value properties_table.PROPERTIES, ''labels'' value labels.label absent on null RETURNING JSON) as vertex from (select distinct * from table(:1)) VT outer apply('
                          || LATERALSTRING
                          || ') properties_table, '
  ...
  WITH EDGES AS (
    SELECT
        JSON_OBJECT('id' VALUE JSON_VALUE(ET.COLUMN_VALUE, '$.ELEM_TABLE')
                               || JSON_QUERY(ET.COLUMN_VALUE, '$.KEY_VALUE'), 'source' IS ID_TABLE.SOURCE, 'target' IS ID_TABLE.TARGET, 'properties' VALUE PROPERTIES_TABLE.PROPERTIES, 'labels' VALUE LABELS.LABEL ABSENT ON NULL RETURNING JSON ) AS EDGE
    FROM
        (
            SELECT
                DISTINCT *
