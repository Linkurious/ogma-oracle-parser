...
CREATE OR REPLACE PACKAGE DBMS_GVT
 ...
 -----------------------------------------------------------------------------
  ...
  FUNCTION BUILD_JSON(
      ...
      GRAPHNAME VARCHAR2,
      GRAPHOWNER VARCHAR2,
      VERTEX_GROUP SYS.ODCINUMBERLIST,
      EDGE_GROUP SYS.ODCINUMBERLIST
  ) RETURN CLOB;
...
WHERE
 ...
 -- cast date type if data type is date
  ...
  BEGIN
    RETURN '24.3.0 (2024-06-05T22:02:52.119185162Z, build: 689686be)';
  END GET_VERSION;
 ...
 -- if there is vertex id, prepare vertex query string like above example query string
  IF (VERTEX_TABLE.COUNT != 0) THEN
    SELECT DISTINCT COLUMN_VALUE BULK COLLECT INTO DISTINCT_VERTEX_TABLE FROM TABLE(VERTEX_TABLE);
 ...
 -- prepare string inside LATERAL()
    ...
    SUB_QUERY_STRING := 'select json_object (''id'' value json_value(VT.COLUMN_VALUE, ''$.ELEM_TABLE'') || json_query(VT.COLUMN_VALUE, ''$.KEY_VALUE''), ''properties'' value properties_table.PROPERTIES, ''labels'' value labels.label, ''groups'' value groups.groups absent on null returning clob) as vertex from (select distinct * from table(:1)) VT outer apply('
                        || LATERALSTRING
                        || ') properties_table, '
                        || ' LATERAL (SELECT JSON_ARRAYAGG(LABEL_NAME) AS LABEL FROM SYS.ALL_PG_ELEMENT_LABELS x WHERE x.OWNER = JSON_VALUE(VT.COLUMN_VALUE, ''$.GRAPH_OWNER'') AND x.GRAPH_NAME = JSON_VALUE(VT.COLUMN_VALUE, ''$.GRAPH_NAME'') AND x.ELEMENT_NAME = JSON_VALUE(VT.COLUMN_VALUE, ''$.ELEM_TABLE'')) LABELS, '
                        || ' LATERAL (SELECT JSON_ARRAYAGG(C2) AS GROUPS FROM (select DISTINCT C2 from (select column_value as c1, rownum as r1 from table(:2)) t1 JOIN (select column_value as c2, rownum as r2 from table(:3)) t2 on t1.r1 = t2.r2 WHERE C1 = VT.COLUMN_VALUE ) ) GROUPS ) ';
    ...
    EXECUTE IMMEDIATE QUERY_STRING INTO VERTEX USING DISTINCT_VERTEX_TABLE, VERTEX_TABLE, VERTEX_GROUP;
  ...
  WITH EDGES AS (
    SELECT
        JSON_OBJECT('id' VALUE JSON_VALUE(ET.COLUMN_VALUE, '$.ELEM_TABLE')
                               || JSON_QUERY(ET.COLUMN_VALUE, '$.KEY_VALUE'), 'source' IS ID_TABLE.SOURCE, 'target' IS ID_TABLE.TARGET, 'properties' VALUE PROPERTIES_TABLE.PROPERTIES, 'labels' VALUE LABELS.LABEL ABSENT ON NULL RETURNING CLOB ) AS EDGE
    FROM
        (
            SELECT
                DISTINCT *
        ...
        LATERAL(
            SELECT
                'VERTEX_1'
                || JSON_OBJECT('V_ID' IS SRC_TABLE.V_ID )                                                                                                                                                                                                   AS SOURCE,
                'VERTEX_2'
                || JSON_OBJECT('V_ID' IS DST_TABLE.V_ID )                                                                                                                                                                                                   AS TARGET
            FROM
                HR.EDGE_1_2           EDGE_TABLE,
                HR.VERTEX_1           SRC_TABLE,
                HR.VERTEX_2           DST_TABLE
 ...
 -- prepare string inside LATERAL()
    ...
    QUERY_STRING := QUERY_STRING
                    || LATERALSTRING;
    QUERY_STRING := QUERY_STRING
                    ||') id_table, LATERAL (SELECT JSON_ARRAYAGG(LABEL_NAME) AS LABEL FROM SYS.ALL_PG_ELEMENT_LABELS x WHERE x.OWNER = JSON_VALUE(ET.COLUMN_VALUE, ''$.GRAPH_OWNER'') AND x.GRAPH_NAME = JSON_VALUE(ET.COLUMN_VALUE, ''$.GRAPH_NAME'') AND x.ELEMENT_NAME = JSON_VALUE(ET.COLUMN_VALUE, ''$.ELEM_TABLE'')) LABELS, '
                    ||' LATERAL (SELECT JSON_ARRAYAGG(C2) AS GROUPS FROM (select DISTINCT C2 from (select column_value as c1, rownum as r1 from table(:2)) t1 JOIN (select column_value as c2, rownum as r2 from table(:3)) t2 on t1.r1 = t2.r2 WHERE C1 = ET.COLUMN_VALUE ) ) GROUPS )'
                    ||' select JSON_ARRAYAGG(edge returning clob) from edges';

    EXECUTE IMMEDIATE QUERY_STRING INTO EDGE USING DISTINCT_EDGE_TABLE, EDGE_TABLE, EDGE_GROUP;
...
CREATE OR REPLACE FUNCTION ORA_SQLGRAPH_TO_JSON (
  ...
  COUNTER                                 NUMBER := 0;
  VERTEX_GROUP                            SYS.ODCINUMBERLIST := SYS.ODCINUMBERLIST();
  EDGE_GROUP                              SYS.ODCINUMBERLIST := SYS.ODCINUMBERLIST();
BEGIN
 ...
 -- Print data fetched by query
  WHILE DBMS_SQL.FETCH_ROWS (CURS_ID) != 0 LOOP
    COUNTER := COUNTER + 1;
    IF ((PAGE_START < 0
    OR PAGE_SIZE <= 0)
    ...
    AND PAGE_START IS NULL) ) THEN
      ...
      ELSE
        FOR I IN 1..VERTEX_ID_COLUMN_LIST.COUNT LOOP
          ...
          VERTEX_TABLE(V2) := L_JSON;
          VERTEX_GROUP.EXTEND();
          VERTEX_GROUP(V2) := VERTEX_ID_COLUMN_LIST(I);
          V2 := V2 + 1;
        END LOOP;
        ...
        FOR I IN 1..EDGE_ID_COLUMN_LIST.COUNT LOOP
          ...
          EDGE_TABLE(E2) := L_JSON;
          EDGE_GROUP.EXTEND();
          EDGE_GROUP(E2) := EDGE_ID_COLUMN_LIST(I);
          E2 := E2 + 1;
        END LOOP;
      END IF;
  ...
  END IF;
    JSON_FILE := DBMS_GVT.BUILD_JSON(VERTEX_TABLE, EDGE_TABLE, COUNTER, GRAPHNAME, GRAPHOWNER, VERTEX_GROUP, EDGE_GROUP);
  RETURN JSON_FILE;
